/**
 * Pure amount conversion helpers.
 *
 ...
 *
 * The module has no framework imports and is safe for client, server, worker,
 * and tests.
 */
...
/**
 * Whether `rawAmount` is greater than or equal to `minRaw`. Non-numeric values
 * are treated as "show" (defensive: never silently suppress a real alert
 * because of a malformed row).
 */
export function isAtLeastRaw(rawAmount: string, minRaw: string): boolean {
  try {
    return BigInt(rawAmount) >= BigInt(minRaw);
  } catch {
    return true;
  }
}
