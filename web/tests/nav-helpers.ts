...
/**
 * Unified nav assertions (PRD: Unified hybrid navigation, issues 01 + 02).
 *
 ...
 * structure of that nav so each page's E2E seam can confirm nav presence, the
 * "Discover" link target, and the Donate Wallet connector (issue 02) without
 * duplicating the assertions. They assert on rendered structure and link
 * targets, not on component internals or Tailwind classes (per the PRD testing
 * philosophy).
 */
 ...
 * `/creator/explore`, plus the retained "Become a Creator" CTA, and the Donate
 * Wallet connector (issue 02) in its default disconnected state. Also asserts
 * the old "How it works" / "Built on Stellar" scroll-spy anchors are gone.
 */
 ...
 * landmark, no logo, no Discover link, no CTA, no Donate Wallet connector, and
 * no mobile menu toggle. The overlay must stay transparent and chrome-free for
