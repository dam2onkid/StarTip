 ...
 * right cluster is auth-aware: unauthenticated visitors see the "Sign in/up"
 * CTA (links to `/login`); authenticated users see a notification bell + an
 * avatar menu (Dashboard, Logout).
 *
 * `expectUnifiedNav` asserts only the structure shared by both auth states:
 * the nav landmark, the logo, the "Discover" link, and the absence of the old
 * scroll-spy anchors, so it can be reused on any route regardless of auth
 * state. The auth-aware right cluster (CTA target, bell, avatar menu, Dashboard
 * link, logout flow) is asserted in the dedicated `tests/nav-auth.spec.ts`.
 * Assertions are on rendered structure and link targets, not on component
 * internals or Tailwind classes (per the PRD testing philosophy).
 */
 ...
 * `/creator/explore`, and that the old "How it works" / "Built on Stellar"
...
/**
 * Asserts the nav is suppressed: no navigation landmark, no Discover link, no
 * CTA, no Donate Wallet connector, no notification bell, no avatar menu
 * trigger, and no mobile menu toggle. Auth pages and the OBS Overlay surface
 * use this helper to stay transparent and chrome-free. Includes the issue 03
 * authed-right-cluster affordances (bell, avatar menu trigger) so they cannot
 * leak onto the overlay surface.
 */
